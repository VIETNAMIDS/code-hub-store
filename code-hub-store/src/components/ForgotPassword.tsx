  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      const { data: checkData, error: checkError } = await supabase.functions.invoke('send-otp', {
        body: { email, checkEmailExists: true }
      });

      if (checkError) {
        let errorMessage = 'Không thể kiểm tra email. Vui lòng thử lại.';
        if (checkError.context?.json) {
          try {
            const errorData = await checkError.context.json();
            if (errorData?.error) {
      ...
      if (data?.error) {
        ...
        toast({
          title: '📧 Đã gửi mã OTP',
          description: 'Vui lòng kiểm tra hộp thư của bạn. Nếu không thấy, hãy check thư rác/spam.',
        });
      }
    } catch (err) {
