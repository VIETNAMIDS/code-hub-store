  ...
  try {
    ...
    switch (action) {
      ...
      case 'delete':
        ...
        // Check if account has orders referencing it
        ...
        if (checkError) {
          console.error('Check orders error:', checkError);
          throw checkError;
        }
        
        if (relatedOrders && relatedOrders.length > 0) {
          return new Response(
            JSON.stringify({ error: 'Không thể xóa tài khoản này vì đã có đơn hàng liên quan. Hãy đánh dấu là "Đã bán" thay vì xóa.' }),
        ...
        if (deleteError) {
          ...
          if (deleteError.code === '23503') {
            return new Response(
              JSON.stringify({ error: 'Không thể xóa tài khoản này vì đã có dữ liệu liên quan.' }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }
          throw deleteError;
